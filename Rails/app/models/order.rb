  ...
  def self.fetch_report(params)
    column, direction = params[:sort].split('-') if params[:sort].present?
      ...
      if params[:startDate].present? && !params[:endDate].present?
        sanitizedDateFilter = ActiveRecord::Base.sanitize_sql_for_conditions(["WHERE created_at >= STR_TO_DATE( ? , '%Y-%m-%d')", params[:startDate]])
      elsif !params[:startDate].present? && params[:endDate].present?
        sanitizedDateFilter = ActiveRecord::Base.sanitize_sql_for_conditions(["WHERE created_at <= STR_TO_DATE( ? , '%Y-%m-%d')", params[:endDate]])
      elsif params[:startDate].present? && params[:endDate].present?
        sanitizedDateFilter = ActiveRecord::Base.sanitize_sql_for_conditions(["WHERE created_at >= STR_TO_DATE( ? , '%Y-%m-%d') AND created_at <= STR_TO_DATE( ? , '%Y-%m-%d')", params[:startDate], params[:endDate]])
      end
      ...
      SQL
      return ActiveRecord::Base.connection.select_all(ActiveRecord::Base.sanitize_sql_array([sql, Current.user.id, Current.user.id])).to_a
  end
